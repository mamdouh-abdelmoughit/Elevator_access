...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  const char* command = doc["command"];
    ...
    // 2. NEW: Remote Control Logic
    else if (strcmp(command, "REMOTE_CONTROL") == 0) {
        const char* action = doc["action"];
        Serial.printf("Remote Action Received: %s\n", action);

        if (strcmp(action, "OPEN_DOOR") == 0) {
            // Trigger Relay 1 (Pin 1 on MCP23017)
            Serial.println("Activating Relay 1 (OPEN DOOR)");
            mcp.digitalWrite(1, LOW); 
            active_relays.push_back({1, millis() + 2000}); // Auto-off after 2s
        }
        else if (strcmp(action, "CLOSE_DOOR") == 0) {
            // Trigger Relay 2 (Pin 2 on MCP23017)
            Serial.println("Activating Relay 2 (CLOSE DOOR)");
            mcp.digitalWrite(2, LOW);
            active_relays.push_back({2, millis() + 2000}); // Auto-off after 2s
        }
        else if (strcmp(action, "SHUTDOWN") == 0) {
            // Trigger Relay 3 - HOLD IT LOW (Permanent OFF until start)
            Serial.println("Activating Relay 3 (SHUTDOWN/CUT POWER)");
            mcp.digitalWrite(3, LOW); 
            // Note: We do NOT add this to active_relays timer, so it stays OFF
        }
        else if (strcmp(action, "START") == 0) {
            // Release Relay 3
            Serial.println("Releasing Relay 3 (START/POWER ON)");
